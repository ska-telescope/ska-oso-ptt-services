...
@test 'OCI: check metadata with spaces in it' {
    ...
    run make -f ../tests/Makefile oci-build OCI_IMAGE=$image_name VERSION=$version OCI_SKIP_PUSH=true
    echo "$output"
    assert_success
...
@test 'OCI: build with additional dev tags' {
    ...
    run make -f ../tests/Makefile oci-build OCI_IMAGE=$image_name CAR_OCI_REGISTRY_HOST=$image_registry VERSION=$version OCI_SKIP_PUSH=true OCI_BUILD_ADDITIONAL_TAGS="$dev_tag_1 $dev_tag_2" CI_COMMIT_SHORT_SHA=$commit_sha
    echo "$output"
