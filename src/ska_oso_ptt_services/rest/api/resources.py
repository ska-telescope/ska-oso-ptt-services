...
def _get_eb_status(eb_id: str, version: str = None) -> Dict[str, Any]:
    ...
    return retrieved_eb.model_dump() if retrieved_eb else None
...
def get_eb_status(eb_id: str, version: int = None) -> Response:
    ...
    """
    retrieved_eb = _get_eb_status(eb_id=eb_id, version=version)
    return retrieved_eb, HTTPStatus.OK
...
def get_eb_status_history(**kwargs) -> Response:
    ...
    with oda.uow as uow:
        eb_ids = uow.ebs_status_history.query(maybe_qry_params, is_status_history=True)
    return eb_ids, HTTPStatus.OK
